...
const wchar_t wWindowName[] = L"Basicify-2";
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ PWSTR lpCmdLine, _In_ int nShowCmd) {
	...
	const wchar_t wClassName[] = L"BasicifyWndClassHost";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
	...
	case WM_CREATE: {
		...
		hMnTxt = CreateWindowEx(0, L"STATIC", L"Basicify-2", WS_CHILD | WS_VISIBLE, 15, 10, 350, 30, hwnd, 0, 0, 0);
		hHelpTxt = CreateWindowEx(0, L"STATIC", L"Which application do you want to Basicify?", WS_CHILD | WS_VISIBLE, 15, 50, 350, 25, hwnd, 0, 0, 0);
